...
export default function PlayerMobile() {
  ...
  if (loading || !joined) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <LiquidBackground />
        <ParticleField count={20} color="rgba(59,130,246,0.3)" />
        <motion.div className="text-center" animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity }}>
          ...
          <p className="font-serif text-xl text-white/50">Подключение...</p>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="relative flex min-h-screen flex-col text-white">
      <LiquidBackground />
      <ParticleField count={15} color="rgba(161,130,50,0.1)" speed={0.4} />

      <div className="relative z-10 flex flex-1 flex-col px-4 py-6 safe-area-inset">
        {/* Status bar */}
        <div className="mb-4 flex items-center justify-between">
          <p className="font-mono text-[10px] uppercase tracking-widest text-white/30">Участник</p>
          <div className="flex items-center gap-2">
            <motion.div className="h-1.5 w-1.5 rounded-full bg-green-400" animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 2, repeat: Infinity }} />
        ...
        <div className="mb-3 flex gap-1">
          {SLIDES.map((s) => (
            <div key={s.id} className={`h-0.5 flex-1 rounded-full transition-all duration-500 ${s.id <= state.currentSlide ? "bg-amber-400/60" : "bg-white/10"}`} />
          ))}
        </div>
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center text-center">
      <motion.div className="mb-8 h-32 w-32 rounded-full"
        style={{ background: "radial-gradient(circle at 35% 35%, #3b82f6, #1e3a8a, #0f172a)", boxShadow: "0 0 80px rgba(59,130,246,0.2)" }}
      ...
      <motion.div className="mt-6 flex gap-1" animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.5, repeat: Infinity }}>
        {[0, 1, 2].map((i) => (
          <motion.div key={i} className="h-2 w-2 rounded-full bg-amber-400/50" animate={{ y: [0, -8, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: i * 0.15 }} />
        ))}
      </motion.div>
    </div>
  );
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center text-center px-2">
      <GlassCard className="w-full max-w-sm p-6" glow={0.3}>
        {slide.year && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="mb-3 inline-block rounded-full border border-amber-400/20 bg-amber-400/10 px-3 py-0.5">
            <span className="font-mono text-xs font-bold text-amber-300">{slide.year}</span>
          </motion.div>
        )}
        <h2 className="mb-2 font-serif text-2xl font-bold text-white/90">{slide.title}</h2>
        {slide.subtitle && <p className="mb-4 text-sm text-amber-300/70">{slide.subtitle}</p>}
        {slide.body && <p className="text-sm leading-relaxed text-white/50 mb-4">{slide.body}</p>}
        {slide.facts && (
          <div className="space-y-2 text-left">
            {slide.facts.map((f, i) => (
              <motion.div key={i} initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.3 + i * 0.1 }} className="flex items-start gap-2">
                <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-amber-400/40" />
...
function SenateActivity({ senate }: { senate: { average: number; count: number } }) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-2">
      <h2 className="font-serif text-xl text-white/80">Власть монарха</h2>
      <p className="text-center text-sm text-white/40">Сколько власти должен иметь царь?</p>
      {!submitted ? (
        <div className="w-full max-w-xs space-y-6">
          ...
          <div className="relative">
            <input type="range" min={0} max={100} value={value} onChange={(e) => setValue(Number(e.target.value))}
              className="slider-liquid w-full h-3 appearance-none rounded-full bg-white/10 outline-none cursor-pointer"
              style={{ background: `linear-gradient(90deg, #3b82f6 ${value}%, rgba(255,255,255,0.1) ${value}%)` }}
            />
          </div>
          <p className="text-center font-serif text-3xl text-amber-300">{value}%</p>
          <GooeyButton variant="gold" className="w-full py-4" onClick={submit}>⚖️ Отправить</GooeyButton>
        </div>
      ) : (
        <GlassCard className="w-full max-w-xs p-6 text-center" glow={0.4}>
          <motion.p initial={{ scale: 0 }} animate={{ scale: 1 }} className="mb-3 font-mono text-sm text-amber-300/80">Голос учтён!</motion.p>
          <p className="font-mono text-xs text-white/30 mb-2">Среднее мнение</p>
          <p className="font-serif text-4xl text-amber-300">{senate.average}%</p>
          <p className="mt-2 font-mono text-xs text-white/30">Ответили: {senate.count} чел.</p>
        </GlassCard>
      )}
    </div>
  );
...
function QuizActivity({ slide, quiz, endpoint }: {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-4 px-2">
      <h2 className="font-serif text-xl text-white/80">Выберите ответ</h2>
          ...
          return (
            <motion.button key={i} onClick={() => answer(i)} disabled={answered !== null}
              className={`relative w-full overflow-hidden rounded-xl border p-4 text-left transition-all ${
                showResult ? (isCorrect ? "border-green-400/50 bg-green-400/10" : isChosen ? "border-red-400/50 bg-red-400/10" : "border-white/5 bg-white/[0.02]")
            ...
            >
              {showResult && (
                <motion.div className="absolute inset-y-0 left-0" style={{ background: isCorrect ? "rgba(34,197,94,0.1)" : "rgba(255,255,255,0.03)" }}
              ...
              <div className="relative flex items-center justify-between">
                <span className={`text-sm ${showResult && isCorrect ? "text-green-400 font-bold" : "text-white/70"}`}>
                  {showResult && isCorrect && "✓ "}{opt.label}
                </span>
                {showResult && <span className="font-mono text-xs text-white/40">{quiz.options[i]}</span>}
              </div>
            </motion.button>
          );
        })}
        ...
        {IND_LABELS.map((label, i) => {
          ...
          return (
            <motion.button key={i} onClick={() => vote(i)} disabled={voted}
              className={`relative w-full overflow-hidden rounded-xl border p-4 text-left transition-all ${voted ? "border-white/10 bg-white/[0.02]" : "border-white/10 bg-white/5 active:bg-white/10"}`}
            ...
            >
              ...
              <div className="relative flex items-center justify-between">
                ...
                {voted && <span className="font-mono text-xs text-white/40">{count} ({Math.round(pct)}%)</span>}
              </div>
            </motion.button>
          );
        })}
        ...
        {[{ v: "48", l: "кораблей", c: "text-blue-400" }, { v: "200", l: "мануфактур", c: "text-green-400" }, { v: "14", l: "рангов", c: "text-amber-400" }].map((item, i) => (
          <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 + i * 0.15, type: "spring" }}>
            <GlassCard className="p-3 text-center">
              ...
              <p className="font-mono text-[9px] text-white/30">{item.l}</p>
            </GlassCard>
          </motion.div>
        ))}
      </div>
